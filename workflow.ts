...
const workflowId = 'workflow.js-bundle-search.v1';

export default defineWorkflow(workflowId, 'JS Bundle Search', (workflow) =>
  workflow
    .description(
    ...
    .buildGraph((ctx: WorkflowExecutionContext) => {
      ...
      if (!appUrl) {
        throw new Error(
          '[workflow.js-bundle-search] Missing required config: workflows.jsBundleSearch.appUrl',
        );
      }
      ...
      const enableNavigate = ctx.getConfig<boolean>(
        'workflows.jsBundleSearch.enableNavigate',
        true,
      );
      ...
      const enableCollectCode = ctx.getConfig<boolean>(
        'workflows.jsBundleSearch.enableCollectCode',
        true,
      );
      const enableRemoteSearch = ctx.getConfig<boolean>(
        'workflows.jsBundleSearch.enableRemoteSearch',
        false,
      );
      const collectMode = ctx.getConfig<string>(
        'workflows.jsBundleSearch.collectMode',
        'priority',
      );

      return sequenceStep('js-bundle-search-root', (root) => {
        if (enableNavigate) {
          root.tool('navigate-app', 'page_navigate', {
            input: {
              url: appUrl,
        ...
        root.step(
          branchStep('auth-extract-branch', 'js_bundle_search_auth_extract_enabled', (branch) => {
            branch
        ...
        root.step(
          branchStep('collect-code-branch', 'js_bundle_search_collect_code_enabled', (branch) => {
            branch
              .predicateFn(() => enableCollectCode)
              .whenTrue(
                toolStep('collect-code', 'collect_code', {
                  input: {
                    url: appUrl,
              ...
              .whenFalse(
                toolStep('skip-collect-code', 'console_execute', {
                  input: {
                    expression:
        ...
        for (const pattern of patterns) {
          ...
          root.tool(`search-${pattern.name}`, 'search_in_scripts', {
            input: {
              keyword,
              isRegex,
              contextLines: 2,
        ...
        root.step(
          branchStep('remote-search-branch', 'js_bundle_search_remote_fetch_enabled', (branch) => {
            branch
              .predicateFn(() => enableRemoteSearch && Boolean(bundleUrl || appUrl))
              .whenTrue(
                toolStep('remote-js-bundle-search', 'js_bundle_search', {
                  input: {
              ...
              .whenFalse(
                toolStep('skip-remote-search', 'console_execute', {
                  input: {
                    expression:
          ...
          input: {
            expression: `(${JSON.stringify({
              ...
              patterns: patterns.map((p) => ({
                name: p.name,
                query: p.query ?? p.regex ?? p.name,
                isRegex: p.isRegex ?? Boolean(p.regex && !p.query),
              })),
              maxMatches,
              strategy: [
                'navigate',
                ...
                'collect_code',
                'search_in_scripts',
                'optional_remote_js_bundle_search',
              ],
            })})`,
          },
        });
